...
func (ctrl *VirtualMachineController) updateNovnc(vm *vmapi.VirtualMachine) (err error) {
	if vm.Spec.HostedNovnc {
		...
		if err = ctrl.updateNovncService(vm); err != nil {
			glog.Warningf("error updating novnc service %s/%s: %v", vm.Namespace, vm.Name, err)
		}
	} else {
		if err = ctrl.deleteNovncPod(vm.Namespace, vm.Name); err != nil {
			glog.Warningf("error deleting novnc pod %s/%s: %v", vm.Namespace, vm.Name, err)
		}
		if err = ctrl.deleteNovncService(vm.Namespace, vm.Name); err != nil {
			glog.Warningf("error deleting novnc service %s/%s: %v", vm.Namespace, vm.Name, err)
		}
		vm2 := vm.DeepCopy()
		vm2.Status.VncEndpoint = ""
		if err = ctrl.updateVMStatus(vm, vm2); err != nil {
			glog.Warningf("error removing vnc endpoint from vm %s/%s: %v", vm.Namespace, vm.Name, err)
		}
	}
	return
}
